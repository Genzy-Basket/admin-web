...
const aggregateItems = (orders) => {
  ...
  for (const order of orders) {
    for (const item of order.items ?? []) {
      const name =
        item.productSnapshot?.name ?? item.productId?.name ?? "Unknown";
      const unit =
        ...
        {order.items?.map((item, i) => (
          <div key={i} className="flex items-center justify-between text-xs">
            ...
            <span className="font-bold text-slate-700">
              {item.quantity} x {item.value} {item.unit}
            </span>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-1">
        {sub.items?.map((item, i) => (
          <div key={i} className="flex items-center justify-between text-xs">
            ...
            <span className="font-bold text-slate-700">
              {item.quantity} x {item.displayLabel}
            </span>
          </div>
        ))}
      </div>
...
const PackingPage = () => {
  ...
  const { orders, loading, fetchOrders, updateStatus, bulkPack } = useOrders();
    ...
    try {
      ...
      const tomorrowSubs = all.filter((s) =>
        s.deliveryDates?.some(
          (dd) =>
            ["upcoming", "packed"].includes(dd.status) &&
            dd.date?.slice(0, 10) === tomorrow,
        ),
      );
  ...
  const ofdOrders = orders.filter((o) => o.orderStatus === "out_for_delivery");
    ...
    tab === "to_pack"
      ...
      : tab === "packed"
        ...
        : tab === "out_for_delivery"
          ? aggregateItems(ofdOrders)
          : tab === "delivered"
            ? aggregateItems(deliveredOrders)
  ...
  const handleBulkDelivered = async () => {
    ...
    await Promise.all(
      packedOrders.map((o) => updateStatus(o.orderId, "delivered")),
    );
    setBulking(null);
  };
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-emerald-800">
                  {toPackOrders.length} orders to pack
                </p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-amber-800">
                    {packedOrders.length + packedSubs.length} packed, ready to
                    dispatch
                  </p>
                </div>
              </div>
      ...
      {tab === "subscriptions" && unpackedSubs.length > 0 && (
        <div className="bg-purple-50 border border-purple-100 rounded-2xl p-4 mb-5">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-purple-800">
                  {unpackedSubs.length} subscriptions to pack
                </p>
            ...
            <button
              onClick={loadSubs}
              className="p-2 rounded-xl border border-purple-200 text-purple-600 hover:bg-purple-100 transition-all"
              title="Refresh"
            >
              <RefreshCw className="w-4 h-4" />
            </button>
        ...
        <>
          ...
          {tab === "subscriptions" &&
            (unpackedSubs.length === 0 ? (
              <Empty
                icon={Repeat}
                title="No subscription deliveries tomorrow"
                subtitle="Active subscriptions with tomorrow's delivery will appear here"
              />
            ) : (
          ...
          {tab === "to_pack" &&
            (toPackOrders.length === 0 ? (
              <Empty
                ...
                subtitle="New orders will appear here when placed"
              />
            ) : (
          ...
          {tab === "packed" &&
            (packedOrders.length === 0 && packedSubs.length === 0 ? (
              <Empty
                icon={PackageCheck}
                title="No packed items"
                subtitle="Orders and subscriptions will appear here after packing"
              />
            ) : (
          ...
          {tab === "out_for_delivery" &&
            (ofdOrders.length === 0 ? (
              <Empty
                ...
                subtitle="Orders marked as out for delivery will appear here"
              />
            ) : (
              <div>
                <h2 className="text-sm font-black text-slate-500 uppercase tracking-wider mb-3">
                  Out for Delivery — {ofdOrders.length}
                </h2>
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {ofdOrders.map((order) => (
                    <OrderCard
                      ...
                      actionIcon={CheckCircle2}
                      onAction={() => {
                        setActingId(order.orderId);
                        updateStatus(order.orderId, "delivered").then(() =>
                          setActingId(null),
                        );
                      }}
                      acting={actingId === order.orderId}
